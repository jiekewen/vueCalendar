...
export default {
  ...
  methods: {
    ...
    setDateInfo: function(dates) {
      ...
      for (var i = 1; i <= num + week; i++) {
        if (i > week) {
          ...
          start =
            this.zeroFill(start[0]) +
            "/" +
            this.zeroFill(start[1]) +
            "/" +
            this.zeroFill(start[2]);
          ...
          var className = "";
          if (
            one[0] == new Date(this.value).getFullYear() &&
            one[1] == new Date(this.value).getMonth() + 1 &&
            one[2] == new Date(this.value).getDate()
          ) {
            className += "on ";
          }
      ...
      this.datatop =
        '<span class="top_month">' +
        this.getMonthText(this.month) +
        '</span><span class="top_year">' +
        this.year +
        "</span>";
    },
    // 选择日期
    clickDate: function(type, day, dates) {
      ...
      //     将子组件的值传给父组件
      //     template：<vueCalendar v-on:childByValue="childByValue" :child="date0"></vueCalendar>
      //     data：name: ""
      ...
      switch (type) {
        ...
        case 3:
          this.setDateInfo(
            (this.month + 1 == 13 ? this.year + 1 : this.year) +
              "/" +
              (this.month + 1 == 13 ? 1 : this.month + 1) +
              "/" +
              1
          );
          break;
